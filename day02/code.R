...
testSafety <- function(x) {
  x <- as.numeric(unlist(strsplit(x, split = " ")))
  diff <- x - lag(x)
  direction <- sum(diff > 0, na.rm = T)
  
  dirFail <- ifelse(direction == 0 | direction == sum(!is.na(diff)), 0, 1)
  stepFail <- sum(abs(diff) > 3, na.rm = T) + sum(diff == 0, na.rm = T)
  
  sum(dirFail, stepFail)
}

sum(sapply(test, testSafety) == 0)

# data
user <- "polks" #sarahpolk for macbook air
day <- "day02"
data <- as.matrix(read.csv(file.path("/Users", user, "Library", "Mobile Documents", 
                                     "com~apple~CloudDocs", "misc", "aoc2024", day, "data.txt"), 
                           header = F))

sum(sapply(data, testSafety) == 0)
...
dampener <- function(x) {
  ...
  safe <- sapply(c(1:length(x)), function(y) {
    newX <-  x[-y]
    diff <- newX - lag(newX)
    direction <- sum(diff > 0, na.rm = T)
...
sum(apply(test, 1, dampener))
sum(apply(data, 1, dampener))
