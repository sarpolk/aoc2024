...
# lists is 3, so the distance between them is 0. The next numbers to pair up are
# 3 and 4, a distance of 1. The fifth-smallest numbers in each list are 3 and 5,
# a distance of 2. Finally, the largest number in the left list is 4, while the
# largest number in the right list is 9; these are a distance 5 apart. To find
# the total distance between the left list and the right list, add up the
...
test <- data.frame(list1 = c(3, 4, 2, 1, 3, 3),
                   list2 = c(4, 3, 5, 3, 9, 3))

rearrange <- function(x){
  x$list1 <- x$list1[order(x$list1)]
  x$list2 <- x$list2[order(x$list2)]
  return(x)
}
list.rearr <- rearrange(test)

list.rearr %>% 
  mutate(diff = list2 - list1) %>% 
  summarise(ans = sum(diff))

# data
data <- read.csv("/Users/sarahpolk/Library/Mobile Documents/com~apple~CloudDocs/misc/aoc2024/day01/data.txt", header = F)
data <- data %>% 
  separate(V1, c("list1", "list2"), sep = "   ") %>% 
  mutate(list1 = as.numeric(list1),
         list2 = as.numeric(list2))

list.rearr <- rearrange(data)

list.rearr %>% 
  mutate(diff = abs(list2 - list1)) %>% 
  summarise(ans = sum(diff))
...
# appear in the right list, so the similarity score does not increase (2 * 0 =
# 0). The fourth number, 1, also does not appear in the right list. The fifth
# number, 3, appears in the right list three times; the similarity score
# increases by 9. The last number, 3, appears in the right list three times; the
# similarity score again increases by 9. So, for these example lists, the
# similarity score at the end of this process is 31 (9 + 4 + 0 + 0 + 9 + 9).
#
# Once again consider your left and right lists. What is their similarity score?

calcSimilarity <- function(x){
  sapply(x$list1, function(y) y * length(which(x$list2 == y)))
}

sum(calcSimilarity(test))
