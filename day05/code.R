...
# it: 75|47, 75|61, 75|53, and 75|29. 47 is correctly second because 75 must be
# before it (75|47) and every other page must be after it according to 47|61,
# 47|53, and 47|29. 61 is correctly in the middle because 75 and 47 are before
# it (75|61 and 47|61) and 53 and 29 are after it (61|53 and 61|29). 53 is
# correctly fourth because it is before page number 29 (53|29). 29 is the only
# page left and so is correctly last. Because the first update does not include
# some page numbers, the ordering rules involving those missing page numbers are
# ignored.
# 
...
cleanData <- function(x) {
  rules <- str_split(x[grep("[|]", x)], "[|]")
  pages <- str_split(x[grep(",", x)], ",")
  
  list(rules, pages)
}

checkPages <- function(x, rules) {
  page <- as.numeric(x)
...
part1 <- function(data) {
  ...
  goodPages <- which(unlist(lapply(pages, checkPages, rules = rules)) == 0)
  sum(as.numeric(unlist(lapply(pages[goodPages], function(x) x[ceiling(length(x) / 2)]))))
}
